...
measure_text :: proc "c" (text: cl.StringSlice, config: ^cl.TextElementConfig, userData: rawptr) -> cl.Dimensions {
    line_width: f32 = 0

    font := GLOBAL_STATE.assets.fonts[FontID(config.fontId)]
...
clay_raylib_render :: proc(render_commands: ^cl.ClayArray(cl.RenderCommand), allocator := context.temp_allocator) {
...
@(private = "file")
draw_rect :: proc(x, y, w, h: f32, color: cl.Color) {
    rl.DrawRectangle(
        i32(math.round(x)),
        i32(math.round(y)),
        i32(math.round(w)),
...
@(private = "file")
draw_rect_rounded :: proc(x,y,w,h: f32, radius: f32, color: cl.Color){
